...
function validateInput(body: unknown): {
  ...
  if (history !== undefined) {
    ...
    if (history.length > MAX_HISTORY_LENGTH) {
      return {
        valid: false,
        error: `Too many messages. Max ${MAX_HISTORY_LENGTH}`,
      };
    }
    for (let i = 0; i < history.length; i++) {
      const msg = history[i];
      if (!msg || typeof msg !== "object")
        return { valid: false, error: `Invalid message at ${i}` };
  ...
  return {
    ...
    data: {
      ...
      conversationHistory:
        (b.conversationHistory as ChatMessage[] | undefined) ?? [],
    },
  };
}
...
export async function POST(request: NextRequest) {
  ...
  if (!geminiKey && !openRouterKey) {
    return NextResponse.json(
      {
        error:
          "AI service not configured. Set GEMINI_API_KEY or OPENROUTER_API_KEY in .env.local",
      },
      { status: 500 },
    );
  }
  ...
  if (profile.credits <= 0) {
    return NextResponse.json(
      {
        error:
          "Insufficient credits. Please upgrade your plan to continue chatting.",
